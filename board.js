...
(function() {
    ...
    kintone.events.on('app.record.detail.show', function(event) {
        ...
        // デバッグモードの判定
        const DEBUG_KEY = 'uno_debug_mode';
        let isDebugMode = sessionStorage.getItem(DEBUG_KEY) === 'true';
        ...
        // デバッグモードがONの場合は、常に「現在のターンの人」になりすます
        if (isDebugMode) {
            if (currentTurnIndex !== -1) {
                myPlayerIndex = currentTurnIndex; 
                myPlayerName = 'デバッグ操作中 (Player ' + currentTurnIndex + ')';
            } else {
                myPlayerName = 'デバッグ操作中 (ゲーム終了)';
            }
        }
        ...
        let debugBtnText = isDebugMode ? "🐛 一人プレイ: ON" : "🐛 一人プレイ: OFF";
        let debugBtnColor = isDebugMode ? "#2ecc71" : "#7f8c8d";

        let html = `
            <div style="position: absolute; top: 10px; right: 10px;">
                <button id="toggle-debug-btn" style="padding: 5px 10px; font-size: 12px; background-color: ${debugBtnColor}; color: white; border: none; border-radius: 5px; cursor: pointer;">
                    ${debugBtnText}
                </button>
            </div>
        ...
        if (debugBtn) {
            debugBtn.addEventListener('click', function() {
                if (isDebugMode) {
                    sessionStorage.setItem(DEBUG_KEY, 'false');
                } else {
                    sessionStorage.setItem(DEBUG_KEY, 'true');
                    alert('一人プレイデバッグモードをONにしました。\n常に現在のターンのプレイヤーの手札が表示されます。');
                }
                location.reload();
            });
        }
