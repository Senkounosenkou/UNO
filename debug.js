...
(function() {
    ...
    kintone.events.on('app.record.detail.show', function(event) {
        ...
        // --- ★ NEW: ゲーム盤面以外の裏側データをすべて非表示にする ---
        const hideFields = [
            'GameID', 'Players', 'Current_Card', 'Deck_Count', 'Turn_Index',
        ...
        // ==========================================
        // ★ NEW: 一人プレイ（デバッグ）モードの判定
        // ==========================================
        let isDebugMode = sessionStorage.getItem('uno_debug_mode') === 'true';
        if (isDebugMode) {
            // デバッグモードONの場合、常に「現在のターン」のプレイヤーになりすます
            myPlayerIndex = currentTurnIndex;
            myPlayerName = (players[currentTurnIndex] ? players[currentTurnIndex].name : `Player ${currentTurnIndex}`) + ' (一人プレイ中)';
        }
        ...
        let html = `
            <div style="position: relative; text-align:center; margin-bottom: 20px;">
                <!-- ★ NEW: デバッグモード切替ボタン -->
                <button id="debug-toggle-btn" style="position: absolute; top: 0; right: 0; padding: 5px 10px; background: ${isDebugMode ? '#e74c3c' : '#7f8c8d'}; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 12px; font-weight:bold;">
                    ${isDebugMode ? '🐛 一人プレイ: ON' : '🐛 一人プレイ: OFF'}
                </button>
        ...
        if (debugBtn) {
            debugBtn.addEventListener('click', function() {
                if (isDebugMode) {
                    sessionStorage.removeItem('uno_debug_mode');
                    alert('一人プレイモードを終了しました。通常の視点に戻ります。');
                } else {
                    sessionStorage.setItem('uno_debug_mode', 'true');
                    alert('一人プレイモードを開始します！\n自動的に「現在のターンのプレイヤー」として操作できるようになります。');
                }
                location.reload();
            });
        }
                ...
                passBtn.addEventListener('click', function() {
                    ...
                    if (canPlayCard(drawCard, currentCard, 0)) {
                        if (confirm(`引いたカードは「${cardName}」でした！\nこのカードはすぐに出すことができます。出しますか？\n（キャンセルを選ぶと手札に加えてパスします）`)) {
                            ...
                            // ★修正: 引いてすぐ出す場合の特殊効果（方向とペナルティ）を計算
                            let nextDirection = currentDirection;
                            let skipCount = 1;
